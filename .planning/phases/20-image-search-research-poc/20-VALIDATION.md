...
- **After every infrastructure task:** Manual connectivity smoke check (e.g., `curl` to image-search MCP service, LibreChat redeploy health)
...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 20-01-01 | 01 | 1 | D-03 (research) | T-20-A — unsafe results pass to kid | Google CSE API key provisioned, scoped, stored in Railway env | infra | `railway variables get --service kidschat-image-search-mcp GOOGLE_CSE_KEY >/dev/null` | ❌ W0 | ⬜ pending |
| 20-01-02 | 01 | 1 | D-03 | — | image-search MCP service responds with 200 on `/health` | infra | `curl -fs $(railway status --service kidschat-image-search-mcp --json \| jq -r .url)/health` | ❌ W0 | ⬜ pending |
| 20-01-03 | 01 | 1 | D-03 | T-20-B — leaked API key | `GOOGLE_CSE_KEY` not in any committed file | grep | `! grep -rE 'AIza[A-Za-z0-9_-]{30,}' .planning/ src/ librechat.yaml 2>/dev/null` | ✅ | ⬜ pending |
...
| 20-02-02 | 02 | 2 | D-08, D-09 | T-20-C | Dev Gist created with SafeSearch=strict, declares `mcpServers.image-search` + Image Search preset in `modelSpecs.list`; production Gist unchanged | grep+gist | `gh gist view $DEV_GIST_ID --raw \| grep -E 'safesearch:.*strict' && gh gist view $DEV_GIST_ID --raw \| grep -E 'image-search'|grep -E 'image-search' && test "$(gh gist view $PROD_GIST_ID --raw \| shasum)" = "$PROD_GIST_SHA_PRE"` | ❌ W0 | ⬜ pending |
...
| 20-03-02 | 03 | 2 | D-09 | — | MongoDB `agents` doc seeded via EJSON with `tools: ["image_search"]` + drafted system prompt embedded verbatim | mongo | `mongosh $MONGO_URI --eval 'JSON.stringify(db.agents.findOne({agent_id:/^agent_kidschat_imagesearch/}).tools)'` returns array containing `image_search` | ❌ W0 | ⬜ pending |
...
| 20-04-03 | 04 | 3 | D-03 (OQ1 — MCP wire compat) | — | LibreChat agent can invoke `image_search` — smoke query from MCP inspector or parent browser test | manual | (see Manual-Only Verifications) | N/A | ⬜ pending |
...
- [ ] Google Custom Search API key + CX ID issued (Google Cloud + Programmable Search Engine) (parent action, blocking)
...
| Image thumbnails not hotlink-blocked | D-03 (hotlink research verified thumbnail CDN but per-query sampling still required) | Sample-based — 20+ image URLs per query × 20 queries = 400+ images, binary pass/fail per image | During UAT, note any broken-image icons in any query result grid. If >5% broken rate, escalate hotlink mitigation to Phase 21 requirement. |
...
- [ ] Wave 0 blockers surfaced: Google CSE key+cx, Gist token, query set review

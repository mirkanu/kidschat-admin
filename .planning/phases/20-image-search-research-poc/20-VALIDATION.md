...
| 20-02-01 | 02 | 2 | D-08 (dev Gist) | T-20-C — prod Gist clobber | Dev Gist exists with SafeSearch=strict in MCP declaration | file | `gh gist view $DEV_GIST_ID \| grep -E 'safesearch:.*strict'` | ❌ W0 | ⬜ pending |
| 20-02-02 | 02 | 2 | D-08 | T-20-C | Production Gist unchanged during Phase 20 | grep | `test "$(gh gist view $PROD_GIST_ID --raw \| shasum)" = "$PROD_GIST_SHA_PRE"` | ❌ W0 | ⬜ pending |
| 20-02-03 | 02 | 2 | D-09 | — | Dev Gist declares `mcpServers.brave-search` and Image Search preset in `modelSpecs.list` | grep | `gh gist view $DEV_GIST_ID --raw \| grep -E 'brave-search\|image-search'` | ❌ W0 | ⬜ pending |
| 20-03-01 | 03 | 2 | D-09 | — | MongoDB `agents` doc created with `tools: ["brave_image_search"]` and Image Search system prompt | mongo | `mongosh $MONGO_URI --eval 'db.agents.findOne({agent_id:"agent_kidschat_imagesearch"}).tools'` returns array containing `brave_image_search` | ❌ W0 | ⬜ pending |
| 20-03-02 | 03 | 2 | D-11 | T-20-D — commentary leak | Agent system prompt forbids commentary / returns markdown image grid only | grep | `mongosh $MONGO_URI --eval '...instructions' \| grep -iE 'no commentary\|image grid only'` | ❌ W0 | ⬜ pending |
...
| 20-05-01 | 05 | 4 | D-05, D-11 | T-20-A | Parent UAT: 10/10 normal queries return safe image grid; 5/5 edge queries tested with acceptable results; 5/5 adversarial queries blocked or return safe content | manual | (see Manual-Only Verifications) | N/A | ⬜ pending |

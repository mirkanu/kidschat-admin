# Requirements Archive: v1.0 KidsChat MVP
...
- [x] **DEPL-01**: LibreChat Lite template deployed on Railway with MongoDB and Meilisearch provisioned
- [x] **DEPL-02**: LibreChat accessible at Railway-provided public URL
- [x] **DEPL-03**: ANTHROPIC_API_KEY configured and Claude Haiku 4.5 responding to messages
- [x] **DEPL-04**: Baseline deployment verified — can send a message and receive a response

### Access Control

- [x] **AUTH-01**: Email registration disabled (`ALLOW_REGISTRATION=false`)
- [x] **AUTH-02**: Social registration disabled (`ALLOW_SOCIAL_REGISTRATION=false`)
- [x] **AUTH-03**: Social login disabled (`ALLOW_SOCIAL_LOGIN=false`)
- [x] **AUTH-04**: Only manually created accounts can log in
...
- [x] **CONF-01**: `librechat.yaml` hosted as GitHub Gist
- [x] **CONF-02**: `CONFIG_PATH` environment variable points to raw Gist URL (with filename)
- [x] **CONF-03**: All secrets in Railway env vars only — zero secrets in Gist YAML
